...
function Explore() {
  ...
  useEffect(() => {
    if (location.state) {
      const { beachID: id, beachName, beachURL: url } = location.state;
      setBeachID(id);
      setBeach(beachName);
      setBeachURL(url);
      setOpen(true);
  ...
  }, [location.state, API_BASE, beachID]);

  function getSurfAttire(waterTempF) {
    if (typeof waterTempF !== "number" || isNaN(waterTempF)) return "Unknown";
    if (waterTempF > 72) return "Rashguard";
    if (waterTempF >= 65 && waterTempF <= 75) return "2mm";
    if (waterTempF >= 62 && waterTempF < 68) return "3/2mm";
    if (waterTempF >= 58 && waterTempF < 63) return "4/3mm";
    return "5/4mm";
  }
              ...
              <div className="card">
                <div className="wind-info">
                  ...
                  <p>
                    ...
                    {degreesToCompass(environment.windDirection)}
                  </p>
                </div>
                <div className="swell-info">
                  <h4>Swell</h4>
                  <p>
                    {environment.swellHeight}ft {environment.swellPeriod}s{" "}
                    {degreesToCompass(environment.swellDirection)}
                  </p>
                </div>
              </div>
            </div>
